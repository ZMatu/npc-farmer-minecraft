...
public class FakePlayerPlugin extends JavaPlugin {
    ...
    if (command.getName().equalsIgnoreCase("npcspawn")) {
        if (args.length < 2) {
            ...
            player.sendMessage("§7Skins disponibles: " + String.join(", ", skins.keySet()));
            return true;
        }

        String npcName = args[0];
        String skinKey = args[1].toLowerCase();

        // Verifica si la skin existe
        if (!skins.containsKey(skinKey)) {
            player.sendMessage("§cSkin no encontrada. Skins disponibles: " + String.join(", ", skins.keySet()));
            return true;
        }

        SkinData skin = skins.get(skinKey);
    ...
    private void spawnFakePlayer(Location loc, String name, SkinData skin) {
        try {
            ...
            // PLAYER_INFO
            PacketContainer infoPacket = protocolManager.createPacket(com.comphenix.protocol.PacketType.Play.Server.PLAYER_INFO);
            ...
            infoPacket.getLists().write(0, list);
